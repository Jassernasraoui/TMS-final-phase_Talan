codeunit 73560 "Tour Execution Management"
{
    ...
    var
        PlanningLines: Record "Planning Lines";
        TourExecutionTracking: Record "Tour Execution Tracking";
        planningheader: Record "Planification Header";
    ...
    begin
        ...
        if PlanningLines.FindSet() then begin
            ...
            repeat
                ...
                // TourExecutionTracking."Driver no" := planningheader."Driver No.";
                TourExecutionTracking."Tournée Terminée" := false;
                TourExecutionTracking.Insert(true);
                // Use the item information from VehicleChargingLine instead of PlanningLines
                TourExecutionTracking."Item No." := VehicleChargingLine."Item No.";
                TourExecutionTracking."item Description" := VehicleChargingLine."Description";
                TourExecutionTracking."Unit of Measure Code" := VehicleChargingLine."Unit of Measure Code";
                TourExecutionTracking."Delivery Address" := VehicleChargingLine."Delivery Address";
                TourExecutionTracking.Insert(true);
        ...
        if VehicleChargingLine.FindSet() then begin
            ...
            repeat
                ...
                if PlanningLines.FindFirst() then begin
                    ...
                    TourExecutionTracking."Tournée Terminée" := false;
                    TourExecutionTracking."Planned Quantity" := VehicleChargingLine."Actual Quantity";
                    ...
                    if VehicleChargingLine."Document Type" = VehicleChargingLine."Document Type"::Purchase then begin
                        TourExecutionTracking."Quantity Returned" := VehicleChargingLine."Actual Quantity";
                        TourExecutionTracking."Quantity Delivered" := 0;
                    end;
    ...
    begin
        ...
        Message('UpdateStopStatus: TourNo=%1, LineNo=%2, NewStatus=%3, Notes=%4', TourNo, LineNo, Format(NewStatus), Notes);
        ...
        if TourExecutionTracking.FindFirst() then begin
            Message('Avant modification: Statut=%1, Heure Livraison=%2', Format(TourExecutionTracking.Status), Format(TourExecutionTracking."Heure Livraison"));
            ...
            Message('Après modification: Statut=%1, Heure Livraison=%2', Format(TourExecutionTracking.Status), Format(TourExecutionTracking."Heure Livraison"));
    ...
    var
        ...
        ClosureWizardPage: Page "Tour Closure Wizard";
        ClosureWizardRec: Record "Planification Header";
    ...
    begin
        ...
        // Ouvrir l'assistant de clôture de tournée
        ClosureWizardRec.Reset();
        if not ClosureWizardRec.Get(TourNo) then
            Error('Impossible d''ouvrir l''assistant de clôture pour la tournée %1.', TourNo);
        ClosureResult := PAGE.RunModal(PAGE::"Tour Closure Wizard", ClosureWizardRec) = ACTION::OK;
        if not ClosureResult then
            exit; // L'utilisateur a annulé ou fermé le wizard
        ...
        PlanificationHeader.Get(TourNo); // Refresh before modify
        PlanificationHeader.Statut := PlanificationHeader.Statut::Completed;
        PlanificationHeader.Modify(true);
